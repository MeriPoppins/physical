...
from posts.serializers import CommentsListSerializer, PostSerializer, PostsListSerializer


class CommentsListApiTestCase(APITestCase):
    def setUp(self) -> None:
        self.post = Post.objects.create(title='post', text='text', created_at=datetime.now(), views=0)
        self.comment = Comment.objects.create(text='text', created_at=datetime.now(), post=self.post)
