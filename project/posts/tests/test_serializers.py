...
from posts.serializers import CommentsListSerializer, CommentSerializer, PostsListSerializer, PostSerializer


class CommentsListSerializersTestCase(TestCase):
    def test_ok(self):
        post = Post.objects.create(title='post', text='text', created_at=datetime.now(), views=0)
        comment = Comment.objects.create(text='text', created_at=datetime.now(), post=post)
